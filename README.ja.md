...
GoogleTestにインスパイアされた、シンプルさと自動テスト登録を重視した軽量なCユニットテストフレームワークです。
...
- **[APIリファレンス](docs/api.ja.md)** — 全アサーション、マクロ、CLIオプション
- **[ユーザーガイド](docs/guide.ja.md)** — フィクスチャ、サブテスト、スキップ等の使い方
- **[内部設計](docs/internals.ja.md)** — アーキテクチャ、設計判断、プラットフォームサポート
- **[ロードマップ](docs/roadmap.ja.md)** — 将来計画（P1+）

## コンパイラサポート

attest 本体と明示型アサーションマクロは C99 で動作します。`ASSERT_EQ` や `EXPECT_EQ` などの型汎用比較マクロは C11 の `_Generic` を要求し、C99 ビルドでは提供されません。
...
WSL2（Linux 6.6.87.2-microsoft-standard-WSL2）上で `Debug` / `Release` 両方の
ビルドを確認しています。`attest_selftest` は 74 件（73 passed / 1 skipped）、
