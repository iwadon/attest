...
- **P0（実装済み）**: コア機能 - C11、基本的なアサーション、CLIフィルタリング、終了コード、フィクスチャ
- **P1（実装済み）**: TAP/JUnit出力、テストタイムアウト、テストスキップ、並列実行
- **P1+（将来）**: C89/90サポート、高度なフィルタリング、パフォーマンス最適化

詳細な仕様については `docs/` ディレクトリを参照してください。

**注意**: 現在の実装にはP0およびP1のすべての機能が含まれています。`docs/` の仕様書は元々の計画フェーズを反映しており、現在の実装状態と一致しない場合があります。
