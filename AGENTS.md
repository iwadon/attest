...
Review `docs/attest_p0_spec.md` before changing behaviour and call out the affected sections in commits. Track upcoming expectations in `docs/attest_p1_plus_spec.md` and tag TODOs with the planned stage (`// TODO(P1):`). Keep new configuration flags disabled by default unless ratified by the spec, and surface any security-sensitive changes (allocators, signal handlers) in the PR checklist.
