...
**引数の順序について：** これらの比較は対称であり、attest はどちらの引数も
「expected（期待値）」「actual（実際の値）」として扱いません。失敗時の出力
は両辺を対等に `lhs` / `rhs` として報告するため、引数の順序はどちらでも
正しい出力になります。詳細は
[ユーザーガイド：比較アサーションの引数順序](guide.ja.md#比較アサーションの引数順序)
を参照してください。
...
マクロ `ATT_HAS_AUTOREG` は、テストがコンストラクタ属性または MSVC の
`.CRT$XCU` セクション経由で自動登録される場合は `1`、そうでない場合は `0` に
なります。手動登録が必要かどうかは `#if !ATT_HAS_AUTOREG` で判定してください。
