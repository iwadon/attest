...
テストの中には、他テストと並行して走らせられないものがあります。カレントディレクトリ、環境変数、固定パスのファイルなど、プロセス全体で共有される状態に触るケースです。現状の回避策は `--jobs=1` で実行するか、`--filter` で別プロセスから問題のテストだけ呼ぶか、のどちらかです。`TEST_SERIAL(Suite, Name)` / `TEST_SERIAL_F(Fixture, Name)` のマーカを追加して、並列ランナーが「並列フェーズ」と「逐次フェーズ」にテストを分けて実行できるようにすれば、逐次テストが他テストとレースしないことを保証できます。

```c
TEST_SERIAL(Env, MutatesPath) {
    setenv("PATH", "/sandbox", 1);
    // ... このテストが実行中は他のテストが走らない ...
}
```

実装前に詰めるべき設計上の選択肢：

- **粒度。** プロセス全体での排他（シンプル）か、名前付きグループ（`TEST_SERIAL_GROUP("fs", ...)`、別グループの逐次テストは並行可）か。実際にグループが必要なユースケースが出るまではプロセス全体排他で十分。
- **スケジューリング。** 逐次テストを並列バッチの前に走らせるか後か。どちらでも良いので、実装が小さくなる方を選ぶ。
- **長時間の逐次テストとの公平性。** 終盤の長い逐次テストはワーカーをアイドルにする。専用の逐次ワーカや並べ替えで緩和できるが、複雑さが増すので実測してから検討。
...
### Human68k の stderr キャプチャ

- DOS `_DUP`/`_DUP2` を使った Human68k の stderr キャプチャ（現状は
  コンパイル時に無効化されており、selftest はスキップされる）
